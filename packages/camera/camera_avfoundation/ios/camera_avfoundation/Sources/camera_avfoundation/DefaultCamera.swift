...
final class DefaultCamera: NSObject, Camera {
  ...
  func getMinExposureTime() -> Int {
    let activeFormat = captureDevice.avDevice.activeFormat
    let minExposure = activeFormat.minExposureDuration
    // Convert CMTime to microseconds
    return max(1, Int((Double(minExposure.value) / Double(minExposure.timescale)) * 1_000_000))
  }
  ...
  func getMaxExposureTime() -> Int {
    ...
    let maxExposure = activeFormat.maxExposureDuration
    // Convert CMTime to microseconds
    return max(1, Int((Double(maxExposure.value) / Double(maxExposure.timescale)) * 1_000_000))
  }
  ...
  func setWhiteBalanceMode(
    ...
    do {
      ...
      case .auto:
        guard captureDevice.isWhiteBalanceModeSupported(.continuousAutoWhiteBalance) else {
          completion(
            .failure(
              PigeonError(
                code: "WHITE_BALANCE_ERROR",
                message: "Continuous auto white balance is not supported",
                details: nil
              )
            )
          )
      ...
      case .locked:
        guard captureDevice.isWhiteBalanceModeSupported(.locked) else {
          completion(
            .failure(
              PigeonError(
                code: "WHITE_BALANCE_ERROR",
                message: "Locked white balance is not supported",
                details: nil
              )
            )
          )
        ...
        let gains = normalizedWhiteBalanceGains(captureDevice.deviceWhiteBalanceGains)
  ...
  func setManualColorTemperature(_ colorTemperature: Int) {
    guard captureDevice.isWhiteBalanceModeSupported(.locked) else {
      reportErrorMessage("Manual color temperature is not supported on this device")
      return
    }

    let colorTemperatureBounds = getColorTemperatureBounds()
    let boundedTemperature = min(
      max(colorTemperature, colorTemperatureBounds.min),
      colorTemperatureBounds.max
    )
    let temperatureAndTint = AVCaptureDevice.WhiteBalanceTemperatureAndTintValues(
      temperature: Float(boundedTemperature),
      tint: 0
    )
    let gains = captureDevice.deviceWhiteBalanceGains(for: temperatureAndTint)
    let normalizedGains = normalizedWhiteBalanceGains(gains)

    do {
      try captureDevice.lockForConfiguration()
      defer { captureDevice.unlockForConfiguration() }
      captureDevice.setWhiteBalanceModeLocked(with: normalizedGains, completionHandler: nil)
      whiteBalanceMode = .locked
    } catch let error as NSError {
  ...
  func getMinColorTemperature() -> Int {
    return getColorTemperatureBounds().min
  }

  func getMaxColorTemperature() -> Int {
    return getColorTemperatureBounds().max
  }

  private func getColorTemperatureBounds() -> (min: Int, max: Int) {
    guard captureDevice.isWhiteBalanceModeSupported(.locked) else {
      return (2000, 8000)
    }

    let maxGain = max(1.0, captureDevice.maxWhiteBalanceGain)
    let coolGains = normalizedWhiteBalanceGains(
    ...
    let warmGains = normalizedWhiteBalanceGains(
      AVCaptureDevice.WhiteBalanceGains(
        redGain: maxGain,
        greenGain: 1.0,
        blueGain: 1.0
    ...
    let coolTemperature = captureDevice.temperatureAndTintValues(for: coolGains).temperature
    let warmTemperature = captureDevice.temperatureAndTintValues(for: warmGains).temperature

    let minCandidate = Int(floor(min(coolTemperature, warmTemperature)))
    let maxCandidate = Int(ceil(max(coolTemperature, warmTemperature)))

    let clampedMin = max(1000, min(12000, minCandidate))
    let clampedMax = max(1000, min(12000, maxCandidate))
    ...
    -> AVCaptureDevice.WhiteBalanceGains
  {
    ...
    let maximumGain = captureDevice.maxWhiteBalanceGain

    return AVCaptureDevice.WhiteBalanceGains(
      redGain: min(maximumGain, max(minimumGain, gains.redGain)),
      greenGain: min(maximumGain, max(minimumGain, gains.greenGain)),
      blueGain: min(maximumGain, max(minimumGain, gains.blueGain))
    )
  }
  ...
  func setFrameRateRange(
    ...
    if minFrameRate <= 0 || maxFrameRate <= 0 || minFrameRate > maxFrameRate {
      completion(
        .failure(
          PigeonError(
            code: "FRAME_RATE_ERROR",
            message: "Invalid frame rate range",
            details: ["min": minFrameRate, "max": maxFrameRate]
          )
        )
      )
      return
    ...
    do {
      try captureDevice.lockForConfiguration()
      captureDevice.activeVideoMinFrameDuration = CMTime(value: 1, timescale: Int32(maxFrameRate))
      captureDevice.activeVideoMaxFrameDuration = CMTime(value: 1, timescale: Int32(minFrameRate))
      captureDevice.unlockForConfiguration()
      completion(.success(()))
    } catch let error as NSError {
      completion(.failure(DefaultCamera.pigeonErrorFromNSError(error)))
    ...
    guard let connection = captureVideoOutput.connection(with: .video) else {
      completion(
        .failure(
          PigeonError(
            code: "VIDEO_STABILIZATION_ERROR",
            message: "No video connection available",
            details: nil
          )
        )
      )
  ...
  func getFocalLength() -> Double {
    // AVFoundation does not expose focal length directly through CaptureDevice protocol.
    return 0.0
  }
  ...
  func setTorchLevel(
    ...
    do {
      ...
      if clampedLevel == 0.0 {
        captureDevice.torchMode = .off
      } else if let avCaptureDevice = captureDevice as? AVCaptureDevice {
        try avCaptureDevice.setTorchModeOn(level: Float(clampedLevel))
      } else {
        captureDevice.torchMode = .on
      }
      captureDevice.unlockForConfiguration()
  ...
  func getTorchLevel() -> Double {
    ...
    if let avCaptureDevice = captureDevice as? AVCaptureDevice {
      return avCaptureDevice.torchMode == .on ? Double(avCaptureDevice.torchLevel) : 0.0
    }
    return captureDevice.torchMode == .on ? 1.0 : 0.0
  }
  ...
  func setColorEffect(
    ...
    // iOS AVFoundation does not provide direct camera color-effect filters.
    completion(.success(()))
  }
