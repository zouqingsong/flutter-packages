...
protocol CaptureDevice: NSObjectProtocol {
  ...
  func deviceWhiteBalanceGains(
    for tempAndTintValues: AVCaptureDevice.WhiteBalanceTemperatureAndTintValues
  ) -> AVCaptureDevice.WhiteBalanceGains
  func temperatureAndTintValues(
    for deviceWhiteBalanceGains: AVCaptureDevice.WhiteBalanceGains
  ) -> AVCaptureDevice.WhiteBalanceTemperatureAndTintValues
  func setWhiteBalanceModeLocked(
    with deviceWhiteBalanceGains: AVCaptureDevice.WhiteBalanceGains,
...
extension CaptureDevice {
  var whiteBalanceMode: AVCaptureDevice.WhiteBalanceMode {
    get { avDevice.whiteBalanceMode }
    set { avDevice.whiteBalanceMode = newValue }
  }

  var deviceWhiteBalanceGains: AVCaptureDevice.WhiteBalanceGains {
    avDevice.deviceWhiteBalanceGains
  }

  var maxWhiteBalanceGain: Float {
    avDevice.maxWhiteBalanceGain
  }

  func isWhiteBalanceModeSupported(_ mode: AVCaptureDevice.WhiteBalanceMode) -> Bool {
    avDevice.isWhiteBalanceModeSupported(mode)
  }

  func deviceWhiteBalanceGains(
    for tempAndTintValues: AVCaptureDevice.WhiteBalanceTemperatureAndTintValues
  ) -> AVCaptureDevice.WhiteBalanceGains {
    avDevice.deviceWhiteBalanceGains(for: tempAndTintValues)
  }

  func temperatureAndTintValues(
    for deviceWhiteBalanceGains: AVCaptureDevice.WhiteBalanceGains
  ) -> AVCaptureDevice.WhiteBalanceTemperatureAndTintValues {
    avDevice.temperatureAndTintValues(for: deviceWhiteBalanceGains)
  }
  ...
  func setWhiteBalanceModeLocked(
    ...
    avDevice.setWhiteBalanceModeLocked(with: deviceWhiteBalanceGains, completionHandler: handler)
  }
}
